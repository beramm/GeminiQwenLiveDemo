...
class GeminiLive:
    ...
    async def start_session(self, audio_input_queue, video_input_queue, text_input_queue, audio_output_callback, audio_interrupt_callback=None):
        config = types.LiveConnectConfig(
            ...
            # for future use when we want to improve the thinking level or enable thoughts
                    ...
                    while True:
                        async for response in session.receive():
                            ...
                            if usage_metadata:
                                ...
                                response_details = getattr(usage_metadata, "response_tokens_details", None)
                                if prompt_details:
                                    usage_event["input_details"] = [
                                        {"modality": str(d.modality), "tokens": d.token_count}
                            ...
                            if server_content:
                                ...
                                if grounding_metadata and getattr(grounding_metadata, "web_search_queries", None):
                                    await event_queue.put({
                                        "type": "grounding_queries",
